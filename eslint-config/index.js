...
function buildReservedWordRules() {
 ...
 */
export default function tsslConfig(options) {
    const { tsconfigRootDir, extraFileExtensions = ['.tssl'], ignores = [], } = options;
    const filePatterns = ['**/*.ts', ...extraFileExtensions.map(ext => `**/*${ext}`)];
    return tseslint.config(...tseslint.configs.recommendedTypeChecked.map(config => ({
        ...config,
        files: filePatterns,
    })), {
        files: filePatterns,
